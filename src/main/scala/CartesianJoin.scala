...
object CartesianJoin {  
  ...
  def main(args: Array[String]) {
    ...
    // Compute expected
    val dist_udf = udf((x: Vector, y: Vector) => model.keyDistance(x, y), DataTypes.DoubleType)
    var expected = dfA.as("a").crossJoin(dfB.as("b")).withColumn("distance",dist_udf(col(s"a.keys"), col(s"b.keys")))
    //expected.printSchema()

    //select statement on this struct 
    expected = expected.select(col("a.primary_key").alias("pk1"),col("b.primary_key").alias("pk2"),col("distance"))
    //expected.printSchema
    expected.write.parquet("/user/alexeys/Meetup2017/baseline_output")
